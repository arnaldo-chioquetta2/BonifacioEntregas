using System.Reflection;
using System.Runtime.InteropServices;

[assembly: AssemblyTitle("Tele Bonifacio")]
[assembly: AssemblyDescription("Gerenciamento de Teleentrega")]
[assembly: AssemblyConfiguration("")]
[assembly: AssemblyCompany("ATC Informatica")]
[assembly: AssemblyProduct("Tele Bonifacio")]
[assembly: AssemblyCopyright("Copyright © 2025")]
[assembly: AssemblyTrademark("")]
[assembly: AssemblyCulture("")]
[assembly: ComVisible(false)]
[assembly: Guid("308c57af-e663-456d-8a2c-188bbd61423d")]
[assembly: AssemblyVersion("4.4.9.0")]
[assembly: AssemblyFileVersion("4.4.9.0")]

// 4.4.9 07/09/26 Correção do envio FTP e tratamento de falhas no backup automático
// 4.4.9 07/09/26 Melhorado diagnóstico de falhas na impressão de etiquetas.
// 4.4.8 05/09/26 Restaurada função de adição de faltas
// 4.4.7 04/09/26 Adicionada categoria aos produtos e filtrar por categoria
// 4.4.6 29/08/26 Preservação do estado da grid na edição
// 4.4.5 24/08/26 Ajuste no login compartilhado
// 4.4.4 24/08/26 Login Compartilhado Balcão e Escritório
// 4.4.3 22/08/26 Teste ODBC
// 4.4.2 22/08/26 Passa pra odbc automaticamente se falhar acesso normal
// 4.4.1 21/08/26 Correção da persistência do campo Atende Whats
// 4.4.0 21/08/26 Log local obrigatório para erros de impressão
// 4.3.9 20/08/26 Correção do filtro de funcionários no campo Liberado por
// 4.3.8 19/07/26 Duplicar etiquetas preservando formatação individual
// 4.3.7 16/08/26 Novo ajuste nas expansões de letras das etiquetas
// 4.3.6 16/08/26 Ajuste nas expansões de letras das etiquetas
// 4.3.5 15/08/26 Ajuste na operação das etiquetas
// 4.3.4 26/07/26 Modo de texto livre com persistência
// 4.3.4 26/07/26 Log remoto para erros de backup
// 4.3.3 25/07/26 Ajuste do ajuste no tamanho das letras das etiquetas2
// 4.3.2 25/07/26 Ajuste no tamanho das letras das etiquetas2
// 4.3.1 24/07/26 Ajuste no termo LOCAL da etiqueta
// 4.3.0 23/07/26 Ajuste no tamanho das letras das etiquetas
// 4.3.0 22/07/26 Log remoto
// 4.2.9 14/07/26 Ajuste nas dimensões das letras das etiquetas
// 4.2.8 13/07/26 Ajustes nas fontes das etiquetas
// 4.2.7 12/07/26 Correção na gravação das etiquetas
// 4.2.6 11/07/26 Outros ajustes na Impressão de etiquetas
// 4.2.5 11/07/26 Mais ajustes na Impressão de etiquetas
// 4.2.4 10/07/26 Ajustes na Impressão de etiquetas
// 4.2.3 08/07/26 Impressão de etiquetas
// 4.2.2 28/04/26 Informação - Liberado por - Nas entregas
// 4.2.2 28/04/26 Botão limpar também limpa o campo de observações - Nas entregas
// 4.2.1 03/04/26 Impressão da anotação por completo
// 4.2.0 27/04/26 Poder colocar letras nos numeros da prateleira
// 4.1.9 23/04/26 Retorno da edição dos códigos da prateleira
// 4.1.8 22/03/26 Edição nos numeros dos códigos de prateleira
// 4.1.7 21/03/26 Alteração do funcionamento dos códigos de prateleira
// 4.1.6 15/03/26 Informação do percentual de gasto com entregas
// 4.1.5 11/03/26 Impressão em lista na prateleira
// 4.1.4 07/03/26 Tres colunas na impressão
// 4.1.3 03/03/26 Ajuste na impressão da prateleira
// 4.1.2 03/03/26 Correção de bug na tela de códigos de prateleira
// 4.1.1 25/02/26 Tela de códigos de pratileira
// 4.1.0 18/02/26 Correção da Aba Produtos e anotações
// 4.0.9 16/02/26 Filtro de data inicial e final para as entregas   
// 4.0.8 15/02/26 Total na pesquisa de entregas
// 4.0.7 14/02/26 Ajustes no filtro das entregas
// 4.0.6 14/02/26 Filtro por datas
// 4.0.5 11/02/26 José passa a ser o motoboy principal
// 4.0.4 23/11/25 Solução pra não embaralhar a grid das faltas[2]
// 4.0.3 20/11/25 Solução pra não embaralhar a grid das faltas
// 4.0.2 Reconstituição dos fontes
// 4.0.1 Ãnotações dinâmicas
// 4.0.0 29/09/25 Retirada a auto-adaptação do form dos documentos
// 3.9.9 04/09/25 Operador Caixa não vê lançamentos de outros
// 3.9.8 14/08/25 Em modo escritório no caixa ver lançamentos de todos
// 3.9.7 11/08/25 Correção da forma de pagamento Vale no relatório de caixa
// 3.9.7 10/08/25 Forma de pagamento Boleto para as entregas
// 3.9.7 10/08/25 Não fecha mais o programa caso não tenha como ver se teve atualização
// 3.9.6 27/07/25 Adaptação do tamanho dos botões do caixa quando em tela grande
// 3.9.5 Previsão para tipo errado nas formas entrou 2 e passa a interpretar como 0 tambem
// 3.9.4 Tratamento de erros na mudança da data do caixa
// 3.9.3 Ajuste no ajuste das colunas das faltas
// 3.9.2 Diminuição das colunas para caber melhor a observação na grid de faltas
// 3.9.1 Tratamento para mudança de largura imprópria na compra pela falta e aumento de logs na opeação

