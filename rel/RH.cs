...
namespace TeleBonifacio.rel
{
    public partial class RH : Form
    {
        ...
        public void GerarRelCaixa()
        {
            ...
            DateTime dataInicial = new DateTime(Math.Max(dtpDataIN.Value.Ticks, primeiroLancamento.Ticks));
            StringBuilder sb = new StringBuilder();
            sb.AppendLine(glo.ComplStr("Relatório de Horas Trabalhadas", 57, 1));
            sb.AppendLine(glo.ComplStr(cmbVendedor.Text, 57, 1));
            string dDatas = $"Período: {dataInicial.ToString("dd/MM/yyyy")} a {dtnDtFim.Value.ToString("dd/MM/yyyy")}";
            ...
            foreach (Lanctos lancto in relcaixaCompleto)
            {
                ...
                {
                    sb.AppendLine($"{lancto.Data:dd/MM/yyyy} DOMINGO");
                }
                else if (lancto.Nome == "FALTA")
                {
                    sb.AppendLine($"{lancto.Data:dd/MM/yyyy} FALTA");
                }
                else
                {
        ...
        //    bool firstEntry = true;
        //    bool isFirstMonth = true;
        //    List<Lanctos> relcaixaComDomingos = new List<Lanctos>();
        //    DateTime dataAtual = dtpDataIN.Value.Date;
        //    while (dataAtual <= dtnDtFim.Value.Date)
        //    {
        //        Lanctos lanctoExistente = relcaixa.FirstOrDefault(l => l.Data.Date == dataAtual);
        //        if (lanctoExistente != null)
        //        {
        //            relcaixaComDomingos.Add(lanctoExistente);
        //        }
        //        else if (dataAtual.DayOfWeek == DayOfWeek.Sunday)
        //        {
        //            relcaixaComDomingos.Add(new Lanctos { Data = dataAtual, Nome = "DOMINGO" });
        //        }
        //        dataAtual = dataAtual.AddDays(1);
        //    }
        //    foreach (Lanctos lancto in relcaixaComDomingos)
        //    {
        //        if (currentMonth != lancto.Data.Month)
        //        {
        //            if (!firstEntry)
