...
// 4.1.0 Correção a Aba Produtos e anotações
...
// 3.9.1 Tratamento para mudança de largura imprópria na compra pela falta e aumento de logs na opeação
// 3.8.7 Filtra os Devedores pela letra ao teclar
...
namespace TeleBonifacio
{
    public partial class OperFalta : Form
    {
        ...
        private void EnsurePlusTab()
        {
            ...
            if (tbFaltas.TabPages.IndexOfKey(TAB_ADD_NAME) >= 0)
                return;

            TabPage tabAdd = new TabPage("+") { Name = TAB_ADD_NAME };
            tbFaltas.TabPages.Add(tabAdd);
        }

        // Mantém compatibilidade com a chamada antiga PreparaAbasUsers(new VendedoresDAO());
        private void PreparaAbasUsers(VendedoresDAO _)
        {
            PreparaAbasUsers();
        }

        private void PreparaAbasUsers()
        {
            string pastaDocs = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Docs");

            if (!Directory.Exists(pastaDocs))
                Directory.CreateDirectory(pastaDocs);

            // 🔹 Garante que a aba "+" esteja presente
            EnsurePlusTab();

            // 🔹 Busca e ordena arquivos de anotações pela data de criação
            string[] arquivosEncontrados = Directory
                .GetFiles(pastaDocs, "Anotacao_*.rtf")
                .OrderBy(f => File.GetCreationTimeUtc(f))
                .ToArray();

            foreach (string caminho in arquivosEncontrados)
            {
                string nome = Path.GetFileNameWithoutExtension(caminho).Replace("Anotacao_", "");

                // Evita duplicar abas se já existirem
                if (tbFaltas.TabPages.Cast<TabPage>().Any(tp => tp.Text.Equals(nome, StringComparison.OrdinalIgnoreCase)))
                    continue;

                TabPage novaAba = new TabPage(nome);
                ...
                // Insere antes da aba "+"
                int idxPlus = tbFaltas.TabPages.IndexOfKey(TAB_ADD_NAME);
                int pos = idxPlus >= 0 ? idxPlus : tbFaltas.TabPages.Count;
                tbFaltas.TabPages.Insert(pos, novaAba);

                // Carrega o conteúdo da anotação
                try
                {
                ...
                {
                    glo.Loga($"Erro ao carregar anotação {nome}: {ex.Message}");
                }
            }

            glo.Loga($"PreparaAbasUsers: carregadas {arquivosEncontrados.Length} anotações.");
        }


        private void TbFaltas_Selected(object sender, TabControlEventArgs e)
        {
            if (e.TabPage.Name == "tabAddAnotacao")
            {
                string nome = Microsoft.VisualBasic.Interaction.InputBox("Digite o nome da anotação:", "Nova Anotação", "");
                if (!string.IsNullOrWhiteSpace(nome))
                {
                    CriarNovaAbaAnotacao(nome);
                }

                // Volta para a aba anterior caso o usuário cancele
                tbFaltas.SelectedIndex = tbFaltas.TabPages.Count - 2;
            }
        }

        private void CriarNovaAbaAnotacao(string nome)
        {
            string pastaDocs = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Docs");
            if (!Directory.Exists(pastaDocs))
                Directory.CreateDirectory(pastaDocs);

            string nomeArquivo = $"Anotacao_{nome}.rtf";
            string caminhoArquivo = Path.Combine(pastaDocs, nomeArquivo);

            // Evita duplicar se já existir
            if (File.Exists(caminhoArquivo))
            {
                MessageBox.Show($"A anotação '{nome}' já existe!", "Aviso",
                                MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            // 🔹 Cria fisicamente o arquivo com conteúdo RTF válido
            File.WriteAllText(caminhoArquivo,
                @"{\rtf1\ansi\deff0{\fonttbl{\f0 Arial;}}\viewkind4\uc1\pard\f0\fs20 \par}");

            // 🔹 Cria o editor e adiciona à aba
            var atcRtf = new AtcCtrl.ATCRTF
            {
                Dock = DockStyle.Fill,
            ...
            // ❌ Removido o atcRtf.Carrega(); — agora o carregamento ocorre ao selecionar a aba

            // Seleciona a nova aba criada
            tbFaltas.SelectedTab = novaAba;
        }
        ...
        private void ConfigurarGrid()
        {
            try
            {
                // 1. Define o modo de auto-ajuste para preencher o espaço.
                // As larguras abaixo serão tratadas como "pesos" proporcionais.
                dataGrid1.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;

                // 2. Oculta as colunas que você não quer ver.
                // É mais seguro usar os nomes das colunas.
                dataGrid1.Columns["ID"].Visible = false;
                dataGrid1.Columns["IDBalconista"].Visible = false;
                dataGrid1.Columns["UID"].Visible = false;
                ...
                // Colunas com peso maior serão mais largas.
                // Usei os seus valores de Width como base para a proporção.
                dataGrid1.Columns["Cont"].FillWeight = 50;
                dataGrid1.Columns["Compra"].FillWeight = 60;
                dataGrid1.Columns["Forn"].FillWeight = 80;
                dataGrid1.Columns["Data"].FillWeight = 80;
                dataGrid1.Columns["Codigo"].FillWeight = 80;
                dataGrid1.Columns["Quant"].FillWeight = 50;
                dataGrid1.Columns["Marca"].FillWeight = 80;
                dataGrid1.Columns["Descricao"].FillWeight = 240; // A coluna mais larga
                ...
                if (dataGrid1.Columns["Valor"].Visible)
                {
                    dataGrid1.Columns["Valor"].FillWeight = 50;
                    dataGrid1.Columns["Valor"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                }

                // 5. A lógica de scaling para "IsLargeScreen" não é mais necessária!
                // O modo Fill já cuida disso de forma muito mais eficiente.

                dataGrid1.Invalidate();
            }
            catch (Exception ex)
            {
                // Usar nomes de coluna também ajuda a depurar. Se um nome estiver errado, o erro será claro.
                ...
                if (!row.Cells[idColumnName].Value.Equals(DBNull.Value))
                {
                    ...
                    else
                    {
                        ...
                        if (oItem != null)
                        {
                            ...
                            else if (oItem is string itemAsString && int.TryParse(itemAsString, out int convertedInt))
                            {
                                itemId = convertedInt;
                            }
                        }
        ...
        private void AcionaComprou(int gID, float Valor)
        {
            var gridState = new
            {
                FirstDisplayedScrollingRowIndex = dataGrid1.FirstDisplayedScrollingRowIndex,
                ColumnWidths = dataGrid1.Columns.Cast<DataGridViewColumn>()
                                    .ToDictionary(c => c.Name, c => c.Width),
                ColumnDisplayIndexes = dataGrid1.Columns.Cast<DataGridViewColumn>()
                                          .ToDictionary(c => c.Name, c => c.DisplayIndex)
            };
            try
            {
                faltasDAO.Comprou(gID, Valor);
            ...
            finally
            {
                ...
                {
                    dataGrid1.BeginInvoke((MethodInvoker)delegate
                    {
                        try
                        {
                            foreach (var col in dataGrid1.Columns.Cast<DataGridViewColumn>())
                            {
                                if (gridState.ColumnWidths.TryGetValue(col.Name, out int width))
                                {
                                    col.Width = width;
                                }
                            }
                            foreach (var col in dataGrid1.Columns.Cast<DataGridViewColumn>())
                            {
                                if (gridState.ColumnDisplayIndexes.TryGetValue(col.Name, out int displayIndex))
                                {
                                    col.DisplayIndex = displayIndex;
                                }
                            }
                            if (gridState.FirstDisplayedScrollingRowIndex >= 0 &&
                                gridState.FirstDisplayedScrollingRowIndex < dataGrid1.Rows.Count)
                            {
                                dataGrid1.FirstDisplayedScrollingRowIndex = gridState.FirstDisplayedScrollingRowIndex;
                            }
                        }
                        catch (Exception ex)
                        {
                            glo.Loga($"Erro ao restaurar estado da grid: {ex.Message}");
                        }
                    });
                }
            }
            ...
            if (!carregando)
            {
                switch (tbFaltas.SelectedIndex)
                {
                    ...
                    default: // Novas abas criadas dinamicamente
                        ...
                        // 🔹 Aba "+" → cria nova anotação
                        if (abaAtual.Name == TAB_ADD_NAME)
                        {
                            string nome = Microsoft.VisualBasic.Interaction.InputBox("Digite o nome da anotação:", "Nova Anotação", "");
                            if (!string.IsNullOrWhiteSpace(nome))
                            {
                        ...
                        // 🔹 Abas dinâmicas → carrega o texto se necessário
                        if (abaAtual.Controls.Count > 0 && abaAtual.Controls[0] is AtcCtrl.ATCRTF atcRtf)
                        {
        ...
        private void dvDevedores_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
            {
                dvDevedores.DataSource = cDaoD.getDados();                
            } else
            {
