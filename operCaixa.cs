...
// 3.8.8 Digita o nr no combo de clientes no caixa mostra o cliente
...
namespace TeleBonifacio
{
    public partial class operCaixa : Form
    {
        ...
        private void CarregaFormas()
        {
            ...
            if (rt.IsLargeScreen())
            {
                buttonWidth = (int)(buttonWidth * 1.5);
                buttonHeight = (int)(buttonHeight * 1.5);
            }
            CarregaForma(ref cFormas, 0, grpCredito, buttonWidth, buttonHeight);
            CarregaFormaDuasLinhas(ref cFormas, 1, grpDebito, buttonWidth, buttonHeight);
        ...
        public void CarregaForma(ref FormasDAO cForma, int tipoForma, GroupBox targetGroupBox, int buttonWidth, int buttonHeight)
        {
            List<tb.Forma> lstFormas = cForma.getFormas(tipoForma, " or Tipo = 2 ");
            int margin = 15;
        ...
        public void CarregaFormaDuasLinhas(ref FormasDAO cForma, int tipoForma, GroupBox targetGroupBox, int buttonWidth, int buttonHeight)
        {
        ...
        private void btEditar_Click(object sender, EventArgs e)
        {
            ...
            if (input != "") 
            {
                ...
                if (DateTime.TryParseExact(input, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out data))
                {
                    ...
                    {
                        glo.LogaErr("Por favor, selecione pelo menos uma linha para editar a data.", "Nenhuma Seleção");
                        return;
                    }
                    string Lista = "";
                    bool haIdsValidos = false; 
                    foreach (DataGridViewRow row in dataGrid1.SelectedRows)
                    {
                    ...
                    if (!haIdsValidos)
                    {
                        glo.LogaErr("Nenhuma das linhas selecionadas possui um ID válido para alterar a data.", "IDs Inválidos");
                        return;
                    }
                    if (Lista.EndsWith(","))
                    {
                    ...
                    {
                        glo.LogaErr("Erro ao construir a lista de IDs. Tente selecionar as linhas novamente.", "Erro Interno");
                        return;
                    }
                    ...
                    try
                    {
                        ...
                        CarregaGrid();
                        Limpar();
                        MessageBox.Show("Datas atualizadas com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    catch (Exception ex) 
                    {
                        glo.LogaErr($"Ocorreu um erro ao tentar alterar as datas: {ex.Message}\nPor favor, tente novamente ou contate o suporte.", "Erro");
                    }
                }
                else
                {
                    glo.LogaErr("Data inválida. Por favor, digite uma data no formato DD/MM/AAAA.", "Erro");
                }
            }
        }        
        ...
        private void CarregaGrid(int idForma = 0)
        {
            ...
            DataTable dados = Caixa.getDados(DT11, dtnDtFim.Value, idForma, sObs, sCliente, sVendedor, sValor, sValorDebito, sDesconto, nrVend);
        ...
        private void cmbCliente_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                ClienteDAO cCli = new dao.ClienteDAO();
