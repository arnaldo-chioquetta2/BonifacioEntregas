...
namespace TeleBonifacio
{
    public partial class operEdContratos : Form
    {
        ...
        private void VeriftpContrato()
        {
            using (OleDbConnection connection = new OleDbConnection(glo.connectionString))
            {
                connection.Open();
                DataTable schema = connection.GetSchema("Columns", new string[] { null, null, "Contratos", "tpContrato" });
                if (schema.Rows.Count == 0)
                {
                    using (OleDbCommand command = new OleDbCommand("ALTER TABLE Contratos ADD COLUMN tpContrato INTEGER", connection))
                    {
        ...
        private void lstClausulas_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!this.Carregando)
            {
                if (lstClausulas.SelectedItem != null)
                {
                    ...
                    {
                        clausulaSelecionadaId = Convert.ToInt32(clausulaSelecionada[0]); 
                        txtEditarAdicionar.Text = clausulaSelecionada[2].ToString();
                        // btnEditClausula.Text = "Editar";
                    }
                }
            }
        }
        ...
        private void txtEditarAdicionar_KeyUp(object sender, KeyEventArgs e)
        {
            btnEditClausula.Enabled = true;
