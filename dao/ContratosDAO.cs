    ...
    public class ContratosDAO
    {
        ...
        public tb.Contrato GetContratoById(int idContrato)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                contrato.Id = Convert.ToInt32(row["ID"]);
                contrato.Descricao = row["Descricao"]?.ToString();
                contrato.IdEntregador = row["idEntregador"] != DBNull.Value ? Convert.ToInt32(row["idEntregador"]) : 0;
                contrato.Valor = row["Valor"] != DBNull.Value ? Convert.ToDecimal(row["Valor"]) : 0;
                contrato.Status = row["Status"]?.ToString();
                contrato.DataInicio = row["DataInicio"] != DBNull.Value ? Convert.ToDateTime(row["DataInicio"]) : DateTime.MinValue;
                contrato.DataTermino = row["DataTermino"] != DBNull.Value ? Convert.ToDateTime(row["DataTermino"]) : DateTime.MinValue;
                contrato.Observacoes = row["Observacoes"]?.ToString();
                contrato.NomeEmpresa = row["NomeEmpresa"]?.ToString();
                contrato.tpContrato = row["tpContrato"] != DBNull.Value ? Convert.ToInt32(row["tpContrato"]) : 1;
                return contrato;
            }
        ...
        public tb.Contrato GetContratoCompleto(int idContrato)
        {
            ...
            if (dtContrato.Rows.Count > 0)
            {
                ...
                string sqlClausulas = $"SELECT Descricao FROM Clausulas WHERE idContrato = {idContrato} Order by ID";
