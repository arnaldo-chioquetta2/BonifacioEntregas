    ...
    public class FornecedorDao : BaseDAO
    {
        ...
        public List<tb.Fornecedor> getForns()
        {
            try
            {
                ...
                query.Append("SELECT IdForn, Nome FROM Fornecedores");
                DataTable dt = DB.ExecutarConsulta(query.ToString());
                List<tb.Fornecedor> fornecs = new List<tb.Fornecedor>();

                foreach (DataRow row in dt.Rows)
                {
                    fornecs.Add(new Fornecedor
                    {
                        Id = Convert.ToInt32(row["IdForn"]),
            ...
            {
                glo.Loga($"Erro em getForns: {ex.Message}");
                return new List<tb.Fornecedor>();
            }
        }
